...
{ 
    ...
    /**
     * redirect
     * REDIRECT METHOD BASED ON GET / POST / PUT / DELETE
     * @return void
     */
...
//* IF THE PAGE IS NOT FOUND WILL CALL VIEW CLASS
...
//* SET THE CLASS VALUE IN VARIABLE
        $instence = new $class;
//* CHECK IF THERE THE LENGTH OF THE ARRAY IS EQUAL 2 THEN CALL THE CLASS WHICH IS SENT BY ANY TYPE OF METHOD
        if (count($class_arr) == 2) {
            call_user_func([$instence, $class_arr[1]]);
        }
//* AUTH AND VIEW METHOD IN MAIN CONTROLER
        $instence->render();
    }
